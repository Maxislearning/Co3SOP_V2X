...
# Stage 2B: Camera -> Scene Occupancy + Target Occupancy (single-frame
# baseline, no Beam Head/loss/Polar transform this stage -- see plan
# effervescent-shimmying-turing.md + 记录something/Stage 2 doc). Scene
# Occupancy GT and per-link Target masks were regenerated for this run in
# Stage 2A (occupancy_prediction_beam/{occupancy_gt,target_occupancy_gt}/).
#
# GPS-free boundary: inter-agent trans2ego-based feature alignment inside
# V2VOccHead._encode_and_fuse is kept as-is (cooperative perception
# mechanism, audited + confirmed in scope -- 任务7 in the experiment log).
# TargetAwareOccHead's forward only ever sees a FRONT/REAR role token, never
# tx_rx_geometry/TX xyz/distance/azimuth.
...
model = dict(
    ...
    pts_bbox_head=dict(
        type='TargetAwareOccHead',
        freeze=True,
        role_dim=32,
        freeze_scene_branch=False,  # overridden per phase (phaseA config sets True)
        lambda_scene=1.0,
        lambda_target=1.0,
        lambda_focal=1.0,
        lambda_dice=1.0,
...
dataset_type = 'CarlaV2VTargetOccCo3SOP'
...
target_occ_root = '/home/admin0/carla_V2V/occupancy_prediction_beam/target_occupancy_gt'
file_client_args = dict(backend='disk')

# Default meta_keys (transform_3d.py's CustomCollect3D) plus tx_rx_geometry/
# beam_eval_meta (eval-only diagnostics, never read by the model's forward)
# and target_role (the *only* thing TargetAwareOccHead actually consumes).
target_occ_meta_keys = (
    ...
    'tx_rx_geometry', 'beam_eval_meta', 'target_role',
)
...
train_pipeline = [
    ...
    dict(type='CustomCollect3D', keys=['img', 'gt_occ', 'gt_target'], meta_keys=target_occ_meta_keys)
]
